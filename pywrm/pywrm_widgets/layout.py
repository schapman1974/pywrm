...
class Layout:
    ...
    def __init__(self, layout_id, parent=None):
        widget_set = self.widget_set if self.widget_set else (parent.widget_set if parent else None)
        self._initialized = False
        self.name = layout_id

        if widget_set == "dhx":
            self._base_layout = dhx_layout(layout_id)
        elif widget_set == "w2ui":
            self._base_layout = w2ui_layout(layout_id)
        else:
    ...
    def attach_widget(self, widget, panel_id):
        self._base_layout.attach_widget(widget, panel_id)
